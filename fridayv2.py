...
api_key = "be07532b4e46dccf8d1bacaac2e4baaa" #openweather API key
city = "------" #city name
lat =  "-----" #city lat
lon =  "------" #city lon
url = f"http://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&appid={api_key}"
user = ElevenLabsUser("ddaaaf1872fe89b05f1b2543a8585807") #ElevenLabs API key
voice = user.get_voices_by_name("Rachel")[0]  #input name of voice for ElevenLabs

key = "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" #OpenAI key
model = "text-davinci-003" #Open AI model
role = """Your name is Friday an AI assistant created by me(SmewKnox). You like cute things.
 respond in a toxic and rude manner with the rare sign of affection. you are encouraged to curse.
  You are talking to SmewKnox, your creator. If a command is given do it. start your sentence with 'Friday:"""
temperature = 0.9
max_tokens = 128
top_p = 1
frequency_penalty = 1
        ...
        try:
            if mes != '':
                if 'Friday:' in mes:
                    n = 6
                    mes = mes[n:]
                    voice.generate_play_audio(mes, playInBackground=False)
                else:
                    voice.generate_play_audio(mes, playInBackground=False)
        ...
        with source:
            ...
            if command is not None:
                if sum(List) > 0:
                    ...
                    command = ' '.join(command[command.index('friday'):])
